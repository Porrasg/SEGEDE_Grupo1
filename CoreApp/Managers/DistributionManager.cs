...
public class DistributionManager
{
    ...
    public void RunMonthlyDistribution(int month, int year)
    {
        ...
        if (scenario == DistributionScenarios.ZeroDemand)
        {
            ...
            _commDistFactory.Create(abortDist);
            _auditManager.LogAction(null, SystemActor.Name, AuditModules.CentralBank, AuditActions.Execute, "tblCommercialDistribution", 0, null, $"Distribution aborted: {scenario}");
            return;
        }
        ...
        _commDistFactory.Create(dist);
        var createdDist = _commDistFactory.RetrieveByMonth(month, year) ?? throw new BusinessException("Failed to retrieve created commercial distribution.");

        _forecastFactory.BlockMonth(month, year, tExec);
        ...
        foreach (var item in assignments)
        {
            ...
            _statementFactory.Create(stmt);

            _forecastFactory.UpdateStatus(item.forecast.Id, ForecastStates.Distributed, tExec);
            ...
            if (buyer != null && !string.IsNullOrWhiteSpace(buyer.Email))
            {
                ...
                _notifFactory.Create(notif);
            }
        }

        _cbFactory.UpdateInventory(rr, tExec);
